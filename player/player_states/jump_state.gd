...
func on_physics_process(delta: float):
	...
	#multiple jumps
	if !character_body_2d.is_on_floor() && GameInputEvents.jump_input() && current_jump_count != max_jump_count:
		character_body_2d.velocity.y = jump_height
		current_jump_count += 1
	...
	# wall clinign state
	if GameInputEvents.wall_cling_input() && character_body_2d.is_on_wall():
